...
def build_create_params(
    ...
    Given an existing TagResponseModel (source object), a destination folder or snippet,
...
def tags(
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve tag objects.
        destination_folder: [DEPRECATED] The destination folder from which to push tag objects.
    ...
    try:
        source_tag_api = Tag(source_client, max_limit=5000)
        ...
        source_objects = source_tag_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} tag objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
    ...
    try:
        destination_client_api = Tag(destination_client, max_limit=5000)
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned tag objects from {context_type} {resolved_source}",
                "sync": True,
            }
