# scm_config_clone/commands/objects/service.py
...
def build_create_params(
    ...
    """
    Construct the dictionary of parameters required to create a new service object.

    Given an existing ServiceResponseModel (source object), a destination folder or snippet,
    and an optional context type, this function builds a dictionary with all necessary fields for creating
    a new service in the destination tenant. It identifies the protocol type
    (TCP/UDP) and uses `model_dump` on a Pydantic model to ensure only valid,
    explicitly set fields are included.

    Args:
        src_obj: The ServiceResponseModel representing the source service object.
    ...
    Returns:
        A dictionary containing the fields required for `Service.create()`.
        This dictionary is validated and pruned by ServiceCreateModel.

    Raises:
        ValueError: If the source object does not contain a valid protocol configuration.
    """
    data = {
        "name": src_obj.name,
    ...
    # Determine which protocol type is set
    if src_obj.protocol.tcp:
        data["protocol"]["tcp"] = {
            "port": src_obj.protocol.tcp.port,
        }
        if src_obj.protocol.tcp.override:
            data["protocol"]["tcp"]["override"] = (
                src_obj.protocol.tcp.override.model_dump(
                    exclude_unset=True,
                    exclude_none=True,
                )
            )
    elif src_obj.protocol.udp:
        data["protocol"]["udp"] = {
            "port": src_obj.protocol.udp.port,
        }
        if src_obj.protocol.udp.override:
            data["protocol"]["udp"]["override"] = (
                src_obj.protocol.udp.override.model_dump(
                    exclude_unset=True,
                    exclude_none=True,
                )
            )
    else:
        raise ValueError(f"No valid protocol configuration found for {src_obj.name}")

    create_model = ServiceCreateModel(**data)
...
def services(
    ...
    """
    Clone service objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving service objects
    ...
    3. Authenticate to the source tenant and retrieve service objects from the given folder.
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve service objects.
        destination_folder: [DEPRECATED] The destination folder from which to push service objects.
    ...
    """
    typer.echo("🚀 Starting service objects cloning...")
    ...
    try:
        source_service_api = Service(source_client, max_limit=5000)
        ...
        source_objects = source_service_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} service objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
    ...
    try:
        destination_client_api = Service(destination_client, max_limit=5000)
    ...
    for src_obj in objects_to_create:
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(
                    f"Service,{src_obj.name},{getattr(src_obj, context_property)}\n"
                )
        ...
        try:
            new_obj = destination_services.create(create_params)
            created_objs.append(new_obj)
            logger.info(f"Created service object in destination: {new_obj.name}")
        except (
            InvalidObjectError,
            MissingQueryParameterError,
            NameNotUniqueError,
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome service objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned service objects from {context_type} {resolved_source}",
                "sync": True,
            }
            result = destination_services.commit(**commit_params)
            job_status = destination_services.get_job_status(result.job_id)
            logger.info(
                f"Commit job ID {result.job_id} status: {job_status.data[0].status_str}"
            )
        except Exception as e:
            logger.error(f"Error committing service objects in destination: {e}")
            raise typer.Exit(code=1)
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info("No new service objects were created, skipping commit.")

    typer.echo("🎉 Service objects cloning completed successfully! 🎉")
