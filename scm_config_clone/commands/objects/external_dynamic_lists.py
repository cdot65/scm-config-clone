# scm_config_clone/commands/objects/external_dynamic_lists.py
...
def build_create_params(
    src_obj: ExternalDynamicListsResponseModel,
    destination: str,
    context_type: str = "folder",
) -> Dict[str, Any]:
    """
    Construct the dictionary of parameters required to create a new EDL object.

    Given an existing ExternalDynamicListsResponseModel (source object), a destination folder or snippet,
    and an optional context type, this function builds a dictionary with all necessary fields for creating
    a new EDL in the destination tenant. It identifies the EDL type and uses
    `model_dump` on a Pydantic model to ensure only valid, explicitly set fields
    are included.

    Args:
        src_obj: The ExternalDynamicListsResponseModel representing the source EDL object.
    ...
    Returns:
        A dictionary containing the fields required for `ExternalDynamicLists.create()`.
        This dictionary is validated and pruned by ExternalDynamicListsCreateModel.

    Raises:
        ValueError: If the source object does not contain a valid EDL type.
    """
    data = {
        "name": src_obj.name,
        context_type: destination,
        "type": src_obj.type.model_dump() if src_obj.type else None,
    }

    if not data["type"]:
        raise ValueError(f"No valid EDL type found for {src_obj.name}")

    create_model = ExternalDynamicListsCreateModel(**data)
...
def external_dynamic_lists(
    ...
    """
    Clone External Dynamic List (EDL) objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving EDL objects
    ...
    3. Authenticate to the source tenant and retrieve EDL objects from the given folder.
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve EDL objects.
        destination_folder: [DEPRECATED] The destination folder from which to push EDL objects.
    ...
    """
    typer.echo("🚀 Starting EDL objects cloning...")
    ...
    try:
        source_edl_api = ExternalDynamicLists(source_client, max_limit=5000)
        ...
        source_objects = source_edl_api.list(**list_params)

        logger.info(
            f"Retrieved {len(source_objects)} EDL objects from source {context_type} '{resolved_source}'."
        )
    except Exception as e:
    ...
    for src_obj in objects_to_create:
        ...
        try:
            new_obj = destination_edls.create(create_params)
            created_objs.append(new_obj)
            logger.info(f"Created EDL object in destination: {new_obj.name}")
        except (
            InvalidObjectError,
            MissingQueryParameterError,
            NameNotUniqueError,
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome EDL objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned EDL objects from {context_type} {resolved_source}",
                "sync": True,
            }
            result = destination_edls.commit(**commit_params)
            job_status = destination_edls.get_job_status(result.job_id)
            logger.info(
                f"Commit job ID {result.job_id} status: {job_status.data[0].status_str}"
            )
        except Exception as e:
            logger.error(f"Error committing EDL objects in destination: {e}")
            raise typer.Exit(code=1)
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info("No new EDL objects were created, skipping commit.")

    typer.echo("🎉 EDL objects cloning completed successfully! 🎉")
