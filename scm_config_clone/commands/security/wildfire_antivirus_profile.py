# scm_config_clone/commands/security/wildfire_antivirus_profile.py
...
def build_create_params(
    ...
    Returns:
        ...
        This dictionary is validated and pruned by WildfireAvProfileCreateModel.
    """
    data = {
        "name": src_obj.name,
    ...
    if src_obj.mlav_exception:
        data["mlav_exception"] = [exc.model_dump() for exc in src_obj.mlav_exception]

    if src_obj.threat_exception:
        data["threat_exception"] = [
            exc.model_dump() for exc in src_obj.threat_exception
        ]

    create_model = WildfireAvProfileCreateModel(**data)
...
def wildfire_antivirus_profiles(
    ...
    """
    typer.echo("🚀 Starting WildFire antivirus profiles cloning...")
    ...
    # Create profiles in destination
    destination_profiles = WildfireAntivirusProfile(destination_client, max_limit=5000)
    created_objs: List[WildfireAvProfileResponseModel] = []
    error_objects: List[List[str]] = []

    for src_obj in objects_to_create:
        if dry_run:
            logger.info(
                f"Skipping creation of WildFire antivirus profile in destination (dry run): {src_obj.name}"
            )
            continue
        ...
        try:
            ...
            logger.info(
                f"Created WildFire antivirus profile in destination: {new_obj.name}"
            )
        except (
            InvalidObjectError,
            MissingQueryParameterError,
    ...
    if created_objs and not quiet_mode:
        typer.echo("\nSuccessfully created the following WildFire antivirus profiles:")
        created_table = []
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome WildFire antivirus profiles failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info(
                "No new WildFire antivirus profiles were created, skipping commit."
            )

    typer.echo("🎉 WildFire antivirus profiles cloning completed successfully! 🎉")
