# scm_config_clone/commands/security/dns_security_profile.py
...
def build_create_params(
    src_obj: DNSSecurityProfileResponseModel,
    destination: str,
    context_type: str = "folder",
) -> Dict[str, Any]:
    """
    Construct the dictionary of parameters required to create a new DNS security profile object.

    Given an existing DNSSecurityProfileResponseModel (source object), a destination folder or snippet,
    ...
    Args:
        src_obj: The DNSSecurityProfileResponseModel representing the source profile object.
    ...
    Returns:
        A dictionary containing the fields required for `DNSSecurityProfile.create()`.
        This dictionary is validated and pruned by DNSSecurityProfileCreateModel.
    """
    data = {
        "name": src_obj.name,
    ...
    if src_obj.botnet_domains:
        data["botnet_domains"] = src_obj.botnet_domains.model_dump(exclude_unset=True)

    create_model = DNSSecurityProfileCreateModel(**data)
...
def dns_security_profiles(
    ...
    """
    Clone DNS security profile objects from a source SCM tenant to a destination SCM tenant.

    This Typer CLI command automates the process of retrieving DNS security profile objects
    ...
    3. Authenticate to the source tenant and retrieve DNS security profile objects from the given folder or snippet.
    ...
    Args:
        ...
        source_folder: [DEPRECATED] The source folder from which to retrieve DNS security profile objects.
        destination_folder: [DEPRECATED] The destination folder from which to push DNS security profile objects.
    ...
    """
    typer.echo("🚀 Starting DNS security profile objects cloning...")
    ...
    try:
        ...
        logger.error(f"Error retrieving DNS security profile objects from source: {e}")
        raise typer.Exit(code=1)
    ...
    # Create DNS security profile objects in destination
    destination_profiles = DNSSecurityProfile(destination_client, max_limit=5000)
    created_objs: List[DNSSecurityProfileResponseModel] = []
    error_objects: List[List[str]] = []

    for src_obj in objects_to_create:
        if dry_run:
            logger.info(
                f"Skipping creation of DNS security profile object in destination (dry run): {src_obj.name}"
            )
            continue
        ...
        if create_report:
            with open("result.csv", "a") as f:
                ...
                f.write(
                    f"DNS Security Profile,{src_obj.name},{getattr(src_obj, context_property)}\n"
                )
        ...
        try:
            ...
            logger.info(
                f"Created DNS security profile object in destination: {new_obj.name}"
            )
        except (
            InvalidObjectError,
            MissingQueryParameterError,
    ...
    if created_objs and not quiet_mode:
        typer.echo("\nSuccessfully created the following DNS security profile objects:")
        created_table = []
    ...
    if error_objects and not quiet_mode:
        typer.echo("\nSome DNS security profile objects failed to be created:")
        typer.echo(
            tabulate(
                error_objects,
    ...
    if commit_and_push and created_objs and context_type == "folder":
        try:
            commit_params = {
                ...
                "description": f"Cloned DNS security profile objects from {context_type} {resolved_source}",
                "sync": True,
            }
            ...
            logger.error(
                f"Error committing DNS security profile objects in destination: {e}"
            )
            raise typer.Exit(code=1)
        ...
        if created_objs and not commit_and_push:
            ...
            logger.info(
                "No new DNS security profile objects were created, skipping commit."
            )

    typer.echo("🎉 DNS security profile objects cloning completed successfully! 🎉")
